...
def RBF3D(unknownpoints, rangeknownpoints):
    x = unknownpoints[0][0]
    y = unknownpoints[0][1]
    z = unknownpoints[0][2]
    ...
    values = rangeknownpoints[:, 3]
    if rangeknownpoints.shape[0] < 5:
        return [x, y, z, np.mean(rangeknownpoints[:, 3]), i, j, k]

    res = griddata(
        rangeknownpoints2, values, unknownpoints[0][:3], method="nearest"
    )  # 插值计算，计算出插值点的值

    # print(res)

    return [x, y, z, res[0], i, j, k]
...
def call_func_job(args):
    ...
    return RBF3D(unknownpoints, rangeknownpoints)
...
if __name__ == "__main__":
    ...
    save_res_path = "./result/res_RBF_zones_{}_{}.txt".format(
